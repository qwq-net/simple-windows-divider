...
機能 C の自動復元はルールを手書きしません。矢印キーでウィンドウを配置したとき、その識別子（実行ファイル名・ウィンドウクラス・AUMID）に対するスロット（どのディスプレイのどの占有範囲か）を学習します。学習データは設定とは別ファイルの `%APPDATA%\windows-divider\layouts.toml` に保存され、自動で生成・更新されます。通常は手で編集する必要はありません。
