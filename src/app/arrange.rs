...
//! 起点はウィンドウの現在矩形から推定する。介入してよいかの判定（[`crate::win::guard`]）は呼び出し側
//! [`super::App`] が済ませてからここへ入る前提。
...
use crate::win::{monitor, window_ops};
...
///
/// `hwnd` は介入可否判定（[`crate::win::guard::should_intervene`]）を通過済みの対象ウィンドウ。待ち時間を
/// 入れず即座に判定するため、単独押しに遅延は乗らない（反対方向は [`GetAsyncKeyState`] で判定）。
pub fn on_arrow(cfg: &Config, hwnd: HWND, family: Family) {
    if opposite_arrow_down(family) {
        apply_axis_full(cfg, hwnd, family.is_horizontal());
    } else {
        apply_arrow(cfg, hwnd, family);
    }
}
...
fn apply_arrow(cfg: &Config, hwnd: HWND, family: Family) {
    let Some((base, work, cols, rows)) = prepare_base(cfg, hwnd) else {
        return;
    };
    let next = grid::step(base, family, cols, rows);
    if next == base && move_to_adjacent_monitor(hwnd, base, family, cols, rows) {
        return;
    }
    set_span(hwnd, next, cols, rows, work);
}

/// `family` 方向の隣モニタへウィンドウを送る。隣が無ければ何もせず `false`。
///
/// 着地は反対側の端セル（[`grid::cross_edge_span`]）。移動先モニタの作業領域へ適用する。
fn move_to_adjacent_monitor(hwnd: HWND, base: GridSpan, family: Family, cols: u32, rows: u32) -> bool {
    ...
    set_span(hwnd, landing, cols, rows, monitors[adj].work_area);
    true
}
...
fn apply_axis_full(cfg: &Config, hwnd: HWND, horizontal: bool) {
    let Some((base, work, cols, rows)) = prepare_base(cfg, hwnd) else {
        return;
    };
...
///
/// 分割数はそのモニタから決める（[`grid_dims`]。自動判定が有効ならアスペクト比から）。起点はウィンドウの
/// 現在矩形から最も近い格子を推定する（自分の配置は見えるウィンドウの縁がセルに合うため、推定で正確に戻る）。
/// OS 最大化中のウィンドウは全グリッド占有を起点にし（最大化から ← で `■■□` などになる）、最大化は解除する。
fn prepare_base(cfg: &Config, hwnd: HWND) -> Option<(GridSpan, Rect, u32, u32)> {
...
/// 占有範囲 `span` を `work` 上の矩形へ適用する。能動的なウィンドウ操作はここに集約する。
fn set_span(hwnd: HWND, span: GridSpan, cols: u32, rows: u32, work: Rect) {
